...
impl Renderer {
    ...
    pub fn new(hwnd: HWND, dpi: u32) -> Result<Renderer, HRESULT> {
        unsafe {
            ...
            // Hwnd render target, sized to the current client rect, DPI from the window
            // (so we work in DIPs and Direct2D scales to physical pixels for us).
            ...
            let fac = d2d_factory.as_raw();
            let hr = ((*(*fac).vtbl).create_hwnd_render_target)(
                fac,
                &rt_props,
                &hwnd_props,
                &mut target,
            ...
            // Brushes.
            let text_brush = ComPtr::from_raw(make_brush(
                target.as_raw(),
                D2D1_COLOR_F { r: 0.10, g: 0.10, b: 0.13, a: 1.0 },
            )?);
            let caret_brush = ComPtr::from_raw(make_brush(
                target.as_raw(),
                D2D1_COLOR_F { r: 0.16, g: 0.40, b: 0.85, a: 1.0 },
            )?);
    ...
    pub fn draw(&mut self, app: &App, caret_visible: bool, px_w: u32, px_h: u32) {
        unsafe {
            ...
            // Ignore D2DERR_RECREATE_TARGET for N0 (device-loss recovery is later).
            let _ = (v.end_draw)(rt, null_mut(), null_mut());
        }
    }
