...
> `SCRIPTORIUM-NATIVE-VIRTUAL-LAYOUT.md`). Ahead of N5, **file I/O is built** (open/save/dirty-state
> with a preserve-on-save codec + a three-way discard guard — `SCRIPTORIUM-NATIVE-IO.md`), because
> loading a real manuscript is the prerequisite for the author's queued feel verdicts. Last
