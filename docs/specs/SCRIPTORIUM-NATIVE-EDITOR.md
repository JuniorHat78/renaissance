...
- **AST-styled rendering — Wave 1 ✅ BUILT (2026-07-06).** `[SPEC → SCRIPTORIUM-NATIVE-STYLING.md]`
  Not a measure-gated node — the author's call to make the in-process parser *visible* (source-highlight:
  keep the markdown markers, style the spans). The N4 worker distills the `Document` into a compact
  `Vec<StyleSpan>` (block spans + kind) carried back under the existing `content_gen` staleness gate;
  the renderer applies font attributes per paragraph **inside `lay_out_paragraph`**, so paint + caret/
  hit-test + the N5 height index all share the styled layout (headings render big+bold and are *taller*,
  and the caret/scroll know it — for free). New range-formatting COM (`SetFontWeight`/`SetFontStyle`/
...
| 2026-07-06 | **AST-styled rendering Wave 1 — the parser made visible (source-highlight, block-level)** | The author's call (not a measured gate): stop rendering flat, apply the parsed structure. **Source-highlight** chosen (markers stay on screen, spans styled) — a faithful source editor (offsets never lie); the hide-markers/WYSIWYG display↔source-mapping model is a deferred landmark. **The AST carries block spans but not inline spans** (inline nodes mirror the JS parser's offset-less serialized output — the equivalence oracle guards it), so **Wave 1 = block-level** (AST as-is: headings sized+weighted by level, block/pull quotes italic) and **Wave 2 = inline** later, when the parser-touch (a parity-safe internal inline `position`) vs re-tokenize fork is actually forced. Mechanism: the N4 worker distills the `Document` into a compact `Vec<StyleSpan>` (not the AST — only the span list crosses the thread) carried back under N4's existing `content_gen` staleness gate (`apply_parse` folds styles with the signal; a stale/out-of-order parse never regresses them). **Styling lives in `lay_out_paragraph`** — the single place a paragraph is shaped — so paint + caret/hit-test + the N5 height index all see the styled layout (the N3 one-authority invariant, virtualized): a heading is taller, the caret + `para_top` + scroll account for it with no extra code. Offsets pick a paragraph's *kind* only; the applied range is always the whole current paragraph, so a stale span mis-tags for ≤1 parse but never reads out of range; no styles yet ⇒ plain (degrade gracefully). New COM `SetFontWeight`(32)/`SetFontStyle`(33)/`SetFontSize`(35) + by-value `DWRITE_TEXT_RANGE` (8 bytes), ABI-asserted + smoke-exercised (the phantom-slot lesson — a heading paragraph must come out taller with the caret still monotonic). **`rust/` lib change:** re-export `Block`/`Inline`/`Position` — purely additive visibility of already-`pub` types so the bin can walk a `Document`; no behaviour change, no effect on the serializer / JS parity / equivalence oracle. **Deviation:** the dialect clamps heading levels to 3 (`parser.rs`), so the §5 `Heading(4..=6)` row never occurs (defensive `Heading(_)` arm). Style-a + Style-b landed as **one commit** (`App.styles` dead until the renderer reads it — the bin-crate reality). Validated: 66 oracles debug+release, styled-paint smoke, ASan-clean across the range-formatting FFI, clippy clean, warning-free, workspace check clean. **Queued for the author: does the styling *look* right** (the size/weight table is a feel knob) + Wave 2 inline + Wave-1.5 color/indent/rules. |
